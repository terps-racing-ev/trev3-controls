...
void read_can_msg(ubyte1 *handle_r, IO_CAN_DATA_FRAME *dst_data_frame,
                  bool *msg_received, ubyte4 id, ubyte1 channel) {
  ...
  *msg_received = FALSE;
  ...
  if (can_read_error == IO_E_OK) {
    ...
    *msg_received = TRUE;
    ...
    IO_CAN_ConfigMsg(handle_r, channel, IO_CAN_MSG_READ, IO_CAN_STD_FRAME, id,
                     0x1FFFFFFF);
  }
}
...
void main(void) {
  ...
  IO_CAN_Init(CAN_CHANNEL, BAUD_RATE, 0, 0, 0);

  IO_CAN_Init(DEBUG_CAN_CHANNEL, BAUD_RATE, 0, 0, 0);

  IO_CAN_ConfigFIFO(&handle_fifo_w, CAN_CHANNEL, FIFO_BUFFER_SIZE,
                    IO_CAN_MSG_WRITE, IO_CAN_STD_FRAME, VCU_CONTROLS_CAN_ID, 0);

  IO_CAN_ConfigMsg(&handle_tms_summary_1_r, CAN_CHANNEL, IO_CAN_MSG_READ,
                   IO_CAN_STD_FRAME, TMS_SUMMARY_1_CAN_ID, 0x1FFFFFFF);

  IO_CAN_ConfigMsg(&handle_tms_summary_2_r, CAN_CHANNEL, IO_CAN_MSG_READ,
                   IO_CAN_STD_FRAME, TMS_SUMMARY_2_CAN_ID, 0x1FFFFFFF);

  IO_CAN_ConfigMsg(&handle_inverter_motor_info_r, CAN_CHANNEL, IO_CAN_MSG_READ,
                   IO_CAN_STD_FRAME, MOTOR_INFO_CAN_ID, 0x1FFFFFFF);

  IO_CAN_ConfigMsg(&handle_inverter_voltage_info_r, CAN_CHANNEL,
                   IO_CAN_MSG_READ, IO_CAN_STD_FRAME, VOLTAGE_INFO_CAN_ID,
                   0x1FFFFFFF);

  IO_CAN_ConfigMsg(&handle_inverter_current_info_r, CAN_CHANNEL,
                   IO_CAN_MSG_READ, IO_CAN_STD_FRAME, CURRENT_INFO_CAN_ID,
                   0x1FFFFFFF);

  IO_CAN_ConfigMsg(&handle_orion_r, CAN_CHANNEL, IO_CAN_MSG_READ,
                   IO_CAN_STD_FRAME, ORION_CAN_ID, 0x1FFFFFFF);

  IO_CAN_ConfigFIFO(&handle_fifo_w_debug, DEBUG_CAN_CHANNEL, FIFO_BUFFER_SIZE,
                    IO_CAN_MSG_WRITE, IO_CAN_STD_FRAME, VCU_CONTROLS_CAN_ID, 0);

  /* rtd with 10k pull-up resistor*/
  bool rtd_val;
  IO_DI_Init(IO_PIN_RTD, IO_DI_PU_10K);
  ...
  /* APPS 1 */
  IO_ADC_ChannelInit(IO_PIN_APPS_1, IO_ADC_RATIOMETRIC, 0, 0, IO_APPS_1_SUPPLY,
                     NULL);

  /* APPS 2 */
  IO_ADC_ChannelInit(IO_PIN_APPS_2, IO_ADC_RATIOMETRIC, 0, 0, IO_APPS_2_SUPPLY,
                     NULL);
  ...
  bool sdc_val;
  IO_DI_Init(IO_PIN_SDC, IO_DI_PD_10K);
  ...
  while (1) {
    ...
    // read tms messages
    read_can_msg(&handle_tms_summary_1_r, &tms_summary_1_can_frame,
                 &tms_summary_1_message_received, TMS_SUMMARY_1_CAN_ID,
                 CAN_CHANNEL);
    read_can_msg(&handle_tms_summary_2_r, &tms_summary_2_can_frame,
                 &tms_summary_2_message_received, TMS_SUMMARY_2_CAN_ID,
                 CAN_CHANNEL);

    // read current info from inverter
    read_can_msg(
        &handle_inverter_current_info_r, &inverter_current_info_can_frame,
        &current_info_message_received, CURRENT_INFO_CAN_ID, CAN_CHANNEL);

    // read message from inverter with motor speed and update motor speed
    // accordingly
    read_can_msg(&handle_inverter_motor_info_r, &inverter_motor_info_can_frame,
                 &motor_info_message_received, MOTOR_INFO_CAN_ID, CAN_CHANNEL);
    if (motor_info_message_received) {
      // d0 and d1 are used for the outgoing CAN message
      last_speed_d0 =
          inverter_motor_info_can_frame.data[INVERTER_MOTOR_SPEED_LO];
      last_speed_d1 =
          inverter_motor_info_can_frame.data[INVERTER_MOTOR_SPEED_HI];
      last_speed = (last_speed_d1 << 8) | last_speed_d0;
      motor_speed_updated_once = TRUE;
    }

    // read message from inverter with voltage and update pack voltage
    // accordingly
    read_can_msg(
        &handle_inverter_voltage_info_r, &inverter_voltage_info_can_frame,
        &voltage_info_message_received, VOLTAGE_INFO_CAN_ID, CAN_CHANNEL);

    // read message from orion
    read_can_msg(&handle_orion_r, &orion_can_frame, &orion_message_received,
                 ORION_CAN_ID, CAN_CHANNEL);
    ...
    if (voltage_info_message_received == TRUE) {
      pack_voltage =
          ((inverter_voltage_info_can_frame.data[INVERTER_PACK_VOLT_HI] << 8) |
           inverter_voltage_info_can_frame.data[INVERTER_PACK_VOLT_LO]) /
          10;
    ...
    if (first_cycle) {
      ...
      if (current_state == NOT_READY) {
        ...
        if (rtd_val == RTD_ON &&
            (IGNORE_RTD_BRAKES ||
             (!bse_error && bse_result > BRAKES_ENGAGED_BSE_THRESHOLD)) &&
            !apps_error && !(sdc_val == SDC_OFF)) {
        ...
        if (just_entered_sound_state) {
          // when this state has just been entered, turn on the buzzer and a
          // timer
        ...
        } else if (!just_entered_sound_state &&
                   IO_RTC_GetTimeUS(rtd_timestamp) > RTD_SOUND_DURATION) {
          // once the timer runs out, enter the new state (driving) and turn off
          // the buzzer
        ...
        if (rtd_val == RTD_OFF) {
        ...
        } else if (!(IGNORE_BRAKE_PLAUSIBILITY) &&
                   bse_result >
                       BRAKE_PLAUSIBILITY_BRAKES_ENGAGED_BSE_THRESHOLD &&
                   apps_pct_result >= APPS_THRESHHOLD_BRAKE_PLAUSIBILITY) {
          ...
          if (torque == 0 && bse_result >= BRAKES_ENGAGED_BSE_THRESHOLD) {
            ...
            // 2's complement to make negative
            regen_torque = !(regen_torque * 10) + 1;
            d0 = regen_torque % 256;
            d1 = regen_torque / 256;
          } else {
            // REGEN: send forward torque request to inverter
            d0 = (torque * 10) % 256;
            d1 = (torque * 10) / 256;
          }
        ...
        if (rtd_val == RTD_OFF) {
          ...
          current_state = ERRORED;
        } else if (apps_pct_result <=
                   APPS_THRESHHOLD_REESTABLISH_PLAUSIBILITY) {
      ...
      if (motor_info_message_received) {
        IO_CAN_WriteFIFO(handle_fifo_w_debug, &inverter_motor_info_can_frame,
                         1);
      }

      if (voltage_info_message_received) {
        IO_CAN_WriteFIFO(handle_fifo_w_debug, &inverter_voltage_info_can_frame,
                         1);
      }

      if (current_info_message_received) {
        IO_CAN_WriteFIFO(handle_fifo_w_debug, &inverter_current_info_can_frame,
                         1);
      }
